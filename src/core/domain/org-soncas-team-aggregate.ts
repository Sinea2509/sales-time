...
export const SONCAS_DRIVER_KEYS = [
  "securite",
  "orgueil",
  "nouveaute",
  "confort",
...
export type SoncasDriverKey = (typeof SONCAS_DRIVER_KEYS)[number];

export type SoncasDriverAverages = Record<SoncasDriverKey, number | null>;
...
export function averageSoncasDriverScores(
  ...
  const sums: Record<SoncasDriverKey, number> = {
    securite: 0,
    orgueil: 0,
    nouveaute: 0,
    confort: 0,
  ...
  const counts: Record<SoncasDriverKey, number> = {
    securite: 0,
    orgueil: 0,
    nouveaute: 0,
    confort: 0,
  ...
  for (const raw of soncasResults) {
    ...
    for (const k of SONCAS_DRIVER_KEYS) {
      sums[k] += d[k].score;
      counts[k] += 1;
    }
  }
