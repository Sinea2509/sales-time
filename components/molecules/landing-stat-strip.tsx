...
export function LandingStatStrip({ stats }: LandingStatStripProps) {
  return (
    <div className="flex w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-white/8 bg-card/4 sm:flex-row">
      {stats.map((stat) => (
        <div
          key={stat.label}
        ...
        >
          ...
          <p className="mt-0.5 text-[11.5px] font-medium text-white/35">
            {stat.label}
          </p>
        </div>
      ))}
