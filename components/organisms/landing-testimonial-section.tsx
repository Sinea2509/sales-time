...
export function LandingTestimonialSection() {
  ...
  return (
    <section id="testi" className="scroll-mt-24 bg-zinc-50 py-[88px]" aria-label="Témoignage">
      <div className="mx-auto max-w-[860px] px-6 sm:px-10">
        <LandingReveal>
          <div className="text-center">
            <div className="mb-7 flex justify-center gap-0.75" aria-hidden>
              {Array.from({ length: 5 }).map((_, i) => (
                <Star key={i} className="size-[18px] fill-amber-600 text-amber-600" />
              ))}
            </div>
            ...
            <div className="mt-[52px] flex flex-col border-y border-zinc-200 sm:flex-row">
              {stats.map((stat) => (
                ...
                >
                  <p className="text-4xl font-bold tracking-tight text-zinc-950">{stat.value}</p>
